---
name: tca-construct-ordered-union
...
```json
{"construct": "ordered union", "name": "LedgerReply", "file": "ledger.py", "axis": "LedgerKind", "members": ["booked", "unbooked"], "variants": ["Booked", "Unbooked"]}
```

`variants` is the attempt order, the failure variant last; each variant is a concept model row pinning one axis member. The row renders as the `Annotated` alias with `union_mode="left_to_right"` and the constructor constant beside it. The row lives in the concept-named file of the foreign thing whose data it constructs.
