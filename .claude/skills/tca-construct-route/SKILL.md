...
```json
{"construct": "route", "name": "FillRoute", "file": "api/position.py", "ingress": "VenueFillMessage", "dispatch": {"verb": "book", "value": "fill"}, "reply": "OrderReceipt"}
```

`ingress` names the contract or foreign model the route constructs from raw transport, each its own row. `dispatch` names the verb and the path to the innermost value it consumes. `reply` names the contract model serialized back, constructed by matching its fields by name against the dispatched value; a route that replies nothing omits it.
