...
```json
{"construct": "binding", "name": "PositionBinding", "file": "service/position.py", "binds": {"bus": "BusClient", "ledger": "LedgerClient"}, "to": "PositionConsistencyModel"}
```

`binds` maps the consistency model's client field names to foreign client classes, and `to` names the consistency model row. Every entry in `binds` must match a client field on the `to` row.
