...
```json
{"construct": "union", "name": "OrderOutcome", "file": "order.py", "axis": "OrderOutcomeKind", "members": ["filled", "rejected"], "variants": ["Filled", "Rejected"]}
```

`axis` is the `StrEnum`, `members` its values, `variants` concept_model rows each pinning a distinct member. The row renders as the `Annotated` alias with the discriminator; no class form exists. The `TypeAdapter` constant renders only when a consumer constructs the union from data with no surrounding modeled structure.
