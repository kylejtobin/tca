...
```json
{"construct": "config", "name": "PositionConfig", "file": "config.py", "env_prefix": "VENUE_", "fields": {"url": "VenueUrl", "token": "SecretStr"}}
```

Every field value names a declared semantic scalar row, or `SecretStr` for a secret. `env_prefix` names the program's environment namespace.
