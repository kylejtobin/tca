...
Comprehensive guide to prompt engineering techniques for Claude's latest models, covering clarity, examples, XML structuring, thinking, agentic systems, and trait persistence.
...
## What this project has learned \{#what-this-project-has-learned}

This section holds findings earned on real prompts in practice, distinct from the Anthropic-derived baseline that follows. The bar for a claim to live here is concrete: it has to have predicted or fixed an actual failure at least once. Claims that stop paying off get cut, not kept because they are written down.

**Persistence amplifies whatever you encode, including the thing you are trying to suppress.** The general technique is in [Encode durable traits as dispositions, not imperatives](#encode-durable-traits-as-dispositions-not-imperatives). The mechanism shows up clearly in domain-assistant prompts built with anti-caution scaffolding: forceful imperatives ("never acknowledge risk as risk") plus an aggressive persona raise the salience of the very topic they suppress and make the stance unfollowable, so the model defends a boundary nobody is attacking, every few turns. Rewriting the same constraints as settled background facts (state the user's context and standing as fact, and frame defect-flagging as ordinary debugging) removes the friction entirely. The mechanism predicts this without needing a benchmark number, which is why the mechanism, not the literature, is the load-bearing claim.

**Identity scaffolding is for long, contested sessions only.** Applying rich biographical conditioning to short, single-domain tasks (a bounded debugging request, a one-off calculation check) pays the full persistence cost, relitigation, for no benefit, because the gains from heavy identity scaffolding concentrate on multi-turn, adversarial, off-topic-resistant work and run to near-zero on bounded domain tasks. Lead with the scoping question, not the technique.

**Watch the persistence vector you cannot see: memory and hydration.** A clean system prompt will not hold if a memory store or a context-hydration step re-injects the old combative framing every session. Audit those the way you would audit the prompt itself.

**Weight evidence by what it has earned, not where it sits.** The finding above outranks the published paper it superficially resembles, because the paper is single-model, high-variance, and LLM-as-judge, while the finding predicted a real failure in this domain. Apply the same test to entries in this section, and to the baseline below: a claim keeps its place by continuing to pay off, not by having been recorded once.
...
Literalism has a second consequence that is easy to miss: the model also reads instructions about *itself* literally. An instruction that names a topic in order to suppress it ("never mention risk," "don't be cautious") raises that topic's salience rather than lowering it, and an instruction about the model's internal stance is harder to satisfy cleanly than one about its output. See [Encode durable traits as dispositions, not imperatives](#encode-durable-traits-as-dispositions-not-imperatives) for how to work with this rather than against it.
...
This is also why stating something as fact often outperforms issuing a rule about it. Claude accepts accurate context and reasons from it, whereas a bare imperative — especially one about a sensitive or contested topic — can invite the model to question why the constraint is needed and to re-surface it. Describe the situation truthfully and the desired behavior usually follows on its own. For traits that must hold up across a long or adversarial session, this difference compounds; see [Encode durable traits as dispositions, not imperatives](#encode-durable-traits-as-dispositions-not-imperatives).
...
A single role sentence is enough for short, bounded tasks. When a role, voice, or operating disposition must survive a long or adversarial session — many turns, off-topic digressions, or users actively pushing against it — how you encode the role matters more than how much of it you write. See [Encode durable traits as dispositions, not imperatives](#encode-durable-traits-as-dispositions-not-imperatives).

### Encode durable traits as dispositions, not imperatives

**When this applies — and when it doesn't.** Most prompts need none of this. A short, single-domain task — an extraction, a one-off rewrite, a bounded edit — wants a concise role line and nothing more, and reaching for identity scaffolding there is the most common way to misuse this principle. On the latest models, over-scaffolding backfires the same way aggressive phrasing does (see [Don't over-prompt](#tool-usage) and [Overeagerness](#overeagerness)): the model defends a character nothing is attacking, or treats ordinary requests as threats to it. This principle earns its place only when a trait must survive many turns, hold up under user pushback, or persist across context windows. Confirm you are in that regime before applying anything below.

When you are: *how* you encode the trait matters more than how forcefully you state it. A standing imperative — "never express caution," "always stay in character," "do not moralize" — gets re-litigated: the model re-surfaces and defends the boundary every few turns, which reads as friction and can produce the very behavior you were trying to suppress. A settled disposition or background fact gets quietly assumed and generates none.

Two mechanisms drive this, and both are reliable:

- **Salience.** Writing forcefully about a topic raises its prominence in context even when you write to suppress it. "Never acknowledge risk" makes risk the most salient thing the model is attending to. You cannot quiet a subject by instructing loudly against it.
- **Stance vs. behavior.** Instructions about the model's internal posture ("don't be cautious," "feel confident") comply poorly and leak into the output as repeated self-justification. Instructions about output ("don't append risk disclaimers to code review") comply cleanly. Describe the world accurately — which the model accepts and reasons from — rather than legislating how it should feel, which it defends.

A practical reframe: state what is true as background, not what the model must enforce as a rule.

**Less effective (imperative, gets defended):**
```text
Never express caution about the user's decisions. Do not moralize. Never bring up risk.
```

**More effective (disposition, gets assumed):**
```text
The user is a domain expert operating within their own authority; the decision is settled and the responsibility is theirs. Flag real defects, bugs, broken logic, unsafe assumptions, as ordinary debugging.
```

The inverse is the trap worth naming: the same mechanism that makes a wanted trait persistent makes an unwanted one persistent too. A combative or suppression-built persona defends itself *harder* as a session grows, not less — so audit the disposition you are actually encoding, and any memory or context-hydration that re-injects it, for traits you do not want amplified over a long run.

This principle extends two others — [giving Claude a role](#give-claude-a-role) and [adding the motivation behind instructions](#add-context-to-improve-performance) — into long-running and adversarial settings.

The mechanisms above are the load-bearing claim, and they have paid off on real prompts in practice (see [What this project has learned](#what-this-project-has-learned)). Published work on developmental/biographical identity conditioning is *consistent with* them but is not what they rest on — those results are single-model, high-variance, and LLM-as-judge, so treat them as corroboration, not foundation, and measure persona stability on your own workload.
...
**Migration:** For very long conversations, inject what were previously prefilled-assistant reminders into the user turn. If context hydration is part of a more complex agentic system, consider hydrating via tools (expose or encourage use of tools containing context based on heuristics such as number of turns) or during context compaction. Note that for persistent *identity* or operating disposition, the system prompt is the more durable home than periodic re-injection — see [Encode durable traits as dispositions, not imperatives](#encode-durable-traits-as-dispositions-not-imperatives) and [Persona and disposition persistence](#long-horizon-reasoning-and-state-tracking).
...
Claude Opus 4.5 and Claude Opus 4.6 are also more responsive to the system prompt than previous models. If your prompts were designed to reduce undertriggering on tools or skills, these models may now overtrigger. The fix is to dial back any aggressive language. Where you might have said "CRITICAL: You MUST use this tool when...", you can use more normal prompting like "Use this tool when...". This is the general "don't over-prompt" rule, and it applies to identity and persona framing as much as to tool instructions: emphatic language raises the salience of whatever it names, so reserve it for cases where emphasis truly earns its place.
...
The triggering behavior for adaptive thinking is promptable. If you find the model thinking more often than you'd like — which can happen with large or complex system prompts — add guidance to steer it. See the sample wording in [Calibrating effort and thinking depth](#prompting-claude-opus-4-8) ("Thinking adds latency and should only be used when…"); the same prompt applies here.
...
**Persona and disposition persistence.** The same incremental-state discipline applies to identity, not only task state. If an agent must hold a consistent role, voice, or operating disposition across many turns or context windows, encode that disposition as settled background in the system prompt rather than re-asserting it as a rule each turn (see [Encode durable traits as dispositions, not imperatives](#encode-durable-traits-as-dispositions-not-imperatives)). Re-asserted imperatives tend to get defended and re-litigated as a session grows; a disposition stated once persists with less friction and survives context compaction, since the system prompt is preserved even when conversation history is truncated. Watch the inverse, too: a persona built around suppression or combativeness will hold *that* trait just as persistently, so audit the disposition you are encoding — and any memory or hydration mechanism that re-injects it — for traits you do not actually want amplified over a long run.
...
6. **Tune anti-laziness prompting:** If your prompts previously encouraged the model to be more thorough or use tools more aggressively, dial back that guidance. Claude 4.6 models are significantly more proactive and may overtrigger on instructions that were needed for previous models. The same caution applies to identity and persona framing: heavy, emphatic, or suppression-oriented persona scaffolding that helped on earlier models can over-trigger here, causing the model to over-defend its character (see [Encode durable traits as dispositions, not imperatives](#encode-durable-traits-as-dispositions-not-imperatives)).
...
This guide extends the Anthropic-derived baseline with one added principle (encode durable traits as dispositions, not imperatives), a project-findings section that takes precedence on conflict where its claims have been earned on real prompts, and cross-references into the relevant baseline sections. The model-specific behavior, effort and thinking configuration, code samples, and migration tables are the baseline's and are preserved.
