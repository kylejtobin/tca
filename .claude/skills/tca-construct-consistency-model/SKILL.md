---
name: tca-construct-consistency-model
...
```json
{"construct": "consistency model", "name": "PositionConsistencyModel", "file": "position.py", "clients": {"bus": "BusClient", "ledger": "LedgerClient"}, "fields": {"latest": "PositionState"}}
```

`clients` maps field names to foreign client classes and is legal only on this row. `fields` maps state field names to declared rows. A row with no state declares `stateless` with the reason; exactly one of `fields` or `stateless` holds. Every client must be reached by at least one verb. Verbs, fields, and clients share one namespace: no name is held twice.
