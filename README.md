...
**[The specification](tca.md)** defines the architecture: the four-layer construction machine and its projection surface, the three mechanisms (wiring, dispatch, orchestration), construction graphs, proof obligations and roots, and the principles that follow from treating construction as proof.
...
When a language model consumes a type schema, field names become instructions and construction becomes proof that the output is valid. TCA already preserves names, descriptions, and enum members as first-class structural elements. Adding an LLM consumer activates a semantic dimension without architectural change: the same types that structure the construction graph become instructions to the model. Section 10 of the specification defines this phenomenon and its information-theoretic consequences.

This is formalized as **[Semantic Index Types](https://github.com/kylejtobin/sit)**, a companion research project exploring what happens when the compilation target reads natural language.
