...
constructing a typed value. The consistency model — the single live node where the graph meets
...
already decides: as a client field on the consistency model when the cognition is in-graph, or
as a projected fact across a boundary when it is cross-process.

**Termination is a union variant constructed, so the consistency model never branches.**
*(Graduated into doctrine. The consistency model section of `docs/type-construction-architecture.md`
now establishes: the consistency model never branches on a bare value; its mutation method body
is a sequence in which each statement is a single legal operation; it constructs the inputs,
matches exhaustively over the narrowed union root, reads the landed variant's own derivation,
and emits. No value-branch anywhere. The proposition below is superseded by that section and
is preserved here only to mark the graduation point.)*

The agent's output type is a union, and the loop ends by the model being forced to land one
...
and forces a guard back into the consistency model's code. The consistency model then constructs the
